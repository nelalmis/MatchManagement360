...
>(
  ...
  async (userId, { rejectWithValue }) => {
    try {
      const leagues = await leagueService.getLeaguesByPlayer(userId);
      return leagues;
    } catch (error: any) {
...
>(
  ...
  async (leagueData, { rejectWithValue }) => {
    try {
      const response = await leagueService.add(leagueData as ILeague);
      if (!response.success) {
...
>(
  ...
  async ({ leagueId, userId }, { rejectWithValue }) => {
    try {
      const success = await leagueService.addPlayerToLeague(leagueId, userId);
      if (!success) {
...
>(
  ...
  async ({ leagueId, userId }, { rejectWithValue }) => {
    try {
      const success = await leagueService.removePlayerFromLeague(leagueId, userId);
      if (!success) {
